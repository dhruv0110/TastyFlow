...
function Slot1Table(props) {
  ...
  const fetchTables = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/slot2');
  ...
  const addTable = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/slot2/add', { number: tableNumber, capacity: tableCapacity });
  ...
  const deleteTable = async (number) => {
    try {
      await axios.delete('http://localhost:5000/api/slot2/delete', { data: { number } });
  ...
  const unreserveTable = async (number) => {
    try {
      ...
      await axios.post('http://localhost:5000/api/slot2/admin/unreserve', 
        { number }, 
        {
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div className='table-show'>
        <h1 className='header'>Manage Tables in Slot - 2</h1>
