...
const getAllSlots = async (req, res) => {
  try {
    const slots = await Slot2.find().populate('reservedBy', 'name contact');
    res.json(slots);
  } catch (error) {
...
const reserveSlot = async (req, res) => {
  try {
    ...
    const slot = await Slot2.findOne({ number });

    if (!slot.reserved) {
      slot.reserved = true;
      slot.reservedBy = userId;
      await slot.save();
      const populatedSlot = await Slot2.findById(slot._id).populate('reservedBy');
      ...
      const mailOptions = {
        ...
        text: `Thank you for reserving a slot. Your Table${slot.number} of slot timing 7:00 to 9:00 is reserved successfully.`
      };
  ...
  try {
    ...
    if (userRole === 'admin' || (slot.reserved && String(slot.reservedBy) === String(userId))) {
      ...
      const mailOptions = {
        ...
        text: `Your reservation for Table${slot.number} of slot timing 7:00 to 9:00 has been canceled. Please book again if needed.`
      };
...
const adminUnreserveSlot = async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      to: reservedByUser.email,
      subject: 'Slot Unreserved',
      text: `Your reservation for Table${slot.number} of slot timing 7:00 to 9:00 has been canceled by the admin. Please book again if needed.`
    };
...
const addSlot = async (req, res) => {
  ...
  try {
    ...
    const newSlot = new Slot2({ number, capacity });
    await newSlot.save();
...
const deleteSlot = async (req, res) => {
  try {
    const { number } = req.body;
    const slot = await Slot2.findOneAndDelete({ number });
    if (!slot) {
      return res.status(404).json({ message: 'Slot not found' });
    }
